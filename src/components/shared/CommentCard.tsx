...
function CommentCard({ name, image, comment }: CommentCard) {
	return (
			...
			<div className='relative z-10 flex flex-col gap-4'>
				<div className='flex gap-6 items-center'>
					<div className='circle-photo rounded-full w-[80px] h-[80px] overflow-hidden border-4 border-[#FF2A68] transform transition-all duration-500 ease-in-out group-hover:scale-110 group-hover:border-[#6a11cb]'>
						<img
							className='client-photo_img object-cover transform transition-transform duration-500 ease-in-out group-hover:scale-110'
							src={image}
							alt={name}
						/>
					</div>
					<h3 className='text-2xl font-semibold text-white group-hover:text-[#FF2A68] transition-all duration-300'>
						{name}
					</h3>
				</div>
				<span className='w-full block h-[2px] my-4 bg-[#FF2A68]'></span>
				<p className='text-sm text-gray-300 group-hover:text-white opacity-80 group-hover:opacity-100 transition-all duration-300'>
					{comment}
				</p>
			</div>
